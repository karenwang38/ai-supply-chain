...
const COMPANIES = {
  ...
  "7610.TW": {
    ...
    relatedIntel: ["2026-06-01-7610-kanghe"],
    notes: `【聯友金屬 (7610) 關鍵數字備忘】
→ 詳細法人報告：市場情報「康和投顧 2026/06/01 深度速報」
─────────────────────────────
■ 2026 Q1 財務爆炸
  • 營收 10.02 億（YoY +484%），單季賺贏 2025 全年
  • 毛利率 74%（2025全年僅 31%），營益率 66%，淨利率 52%
  • 單季 EPS 10.87 元（TTM ~16.32元）

■ 雙軌商業模式
  • 自售模式：95.7%（採購廢料加工後賺價差）
  • 加工代工：4.3%（客戶供料，收工費 + 固定取 6-10% 鈷副產品）

■ 估值動態（股價 1,600 元）
  • 歷史 P/E：~98x（近四季 EPS~16.32）
  • 動態 P/E：~40x（若年化Q1 EPS~40元）
  • PEG：~1.14（G=35%），已突破 1.0 警戒線
  • P/B：>60x（財務健診通過率僅 17%！）

■ 產品時間表
  • 2026/07：新設備量產（關鍵斜率節點）
  • 2026：APT + 氧化鎢產線完成（+26.1%計價毛利）
  • 2027/09：屏科廠（鎢粉+碳化鎢），產能+100%，投資6.5億

■ 危險三大炸彈
  1. 廢清法起訴（屏東地檢署 2025/08，尚未審結）
  2. 董事長+總經理個人擔保銀行借款 11.22億（關鍵人風險）
  3. 8億CB + 2,000仟股現增核准 → 籌碼稀釋壓力

■ 護城河
  • 中國「一單一證」管制 → 亞洲唯一非紅供應鏈
  • M系列低成本庫存 → 抵抗鎢價回檔30-40%
  • 中國出口增值稅差 13%（天然成本利差）`,
    challenges: [
